...
const IMAGE = [
  ...
  },
  {
    name: "Visa",
    image:
      "https://xinvisavietnam.net/wp-content/uploads/2018/10/dich-vu-visa.jpg",
  },
  {
    name: "COD",
    image:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSU3i9dKK73UudhX1ud5XCYt1-fTFj4M2PFtw&usqp=CAU",
  },
  {
    name: "ZaloPay",
    image:
...
function PaymentComponent({ data, isSelected, changeSelect }) {
  ...
  return (
    ...
    >
      {isSelected == data._id ? (
        <FontAwesome name="dot-circle-o" size={28} color="#2196F3" />
      ) : (
...
export default function CreateOrder() {
  ...
  const customerInfo = customerContext.state.address.find(
  ...
  const createOrderDetail = async (productId, quantity, orderId) => {
    let errorMsg = "";
    try {
      const response = await orderApi.createOrderDetail(
        productId,
        quantity,
        orderId
      );
