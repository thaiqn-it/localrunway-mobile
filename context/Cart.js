...
const itemProcessReducer = (state,action) => {
    ...
    if (action.type == "DELETE_ITEM"){
        ...
        const checkExist = state.item.findIndex((item) => item.product._id == action.item.product._id) != -1;   
    ...
    switch (type) {
        ...
        case "UPDATE_ITEM" :
            const product = state.item.find((item) => item.product._id == action.item.product._id)
        ...
        case "DELETE_ITEM" :
            action.item.item.map((id) => {
                state.item.splice(state.item.findIndex((item) => item.product._id == id),1)
            })
            return {
                ...state
            }    
